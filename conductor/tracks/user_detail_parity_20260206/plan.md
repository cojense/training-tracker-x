...
- [ ] Task: Audit `src/views/Profile.tsx` to identify reusable logic and components.
- [ ] Task: Create `src/views/UserDetailView.tsx` to fetch and display data based on the `:id` URL parameter.
- [ ] Task: Update `src/utilities/api.ts` to include methods for fetching another user's assignments and record (`/users/:id/assignments`, `/users/:id/record`).
- [ ] Task: Register the new route `/users/:id` in `App.tsx` with appropriate role-based protection.
...
- [ ] Task: Update `UserListView.tsx` to add a "View" action button linking to `/users/:id`.
- [ ] Task: Update `ManagerReportView.tsx` to make the "Member" column a link to `/users/:member_id`.
- [ ] Task: Update `SupervisorView.tsx` to make the "Member" column a link to `/users/:member_id`.
- [ ] Task: Update `ApprovalQueueView.tsx` to make the "User" column a link to `/users/:user_id`.
